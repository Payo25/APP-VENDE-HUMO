  ...
  try {
    ...
    await pool.query(`
      CREATE TABLE IF NOT EXISTS vacation_profiles (
        ...
        vacation_hourly_rate DECIMAL(8,2),
        notes TEXT,
        createdat TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        lastmodified TIMESTAMP DEFAULT CURRENT_TIMESTAMP
      );
    ...
    // Add vacation_hourly_rate column if missing
    await pool.query(`
...
app.post('/api/invoices/:id/send-email', requireRole('Business Assistant'), invoiceUpload.single('attachment'), async (req, res) => {
  ...
  if (emailList.length === 0) return res.status(400).json({ error: 'At least one valid email is required' });
  ...
  try {
    ...
    <div style="max-width:700px;margin:0 auto;font-family:Arial,sans-serif;color:#333">
      ...
      ${inv.notes ? `<div style="background:#f5f7ff;padding:12px 16px;border-radius:8px;margin-top:16px"><p style="margin:0;font-weight:700;color:#1a237e;font-size:12px;text-transform:uppercase;letter-spacing:1px;margin-bottom:6px">Notes</p><p style="margin:0;font-size:13px;color:#555">${escapeHtml(inv.notes)}</p></div>` : ''}
    ...
    res.json({ success: true, message: `Invoice emailed to ${emailList.join(', ')} (${emailList.length} recipient${emailList.length > 1 ? 's' : ''})` });
  } catch (err) {
